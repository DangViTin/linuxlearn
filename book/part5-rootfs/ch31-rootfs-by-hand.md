...
> **Why:** Chapter 29's BusyBox initramfs was a toy — one cpio file, no persistence, no real /etc. The same machinery becomes a *real* rootfs the moment you (a) populate `/etc/` with config, (b) include shared libraries so dynamically-linked tools work, (c) host it on NFS so you can iterate on user space without reflashing.
> **Focus:** the **handful of files in `/etc/`** that everything else depends on — `inittab`, `init.d/rcS`, `fstab`, `passwd`, `group`. Master these and every higher-level rootfs system (Buildroot, Yocto, Ubuntu-base) becomes "the same content, generated by tools."
...
The host's filesystem *is* the target's filesystem. You edit, the target sees. **This is the iteration speed that makes embedded Linux feel reasonable.**
