...
> **Why:** for most users, `apt install gcc-arm-linux-gnueabihf` is fine. So why build your own? Three real reasons:
...
> 3. **Understanding** — every "weird linker error" makes sense once you've built the linker. Educational value matches Ch 11 (boot ROM image) and Ch 14 (DDR init).
> **Focus:** **the multi-stage build resolves the chicken-and-egg: stage 1 gcc (no libc, can only compile freestanding code) → kernel headers → glibc (compiled by stage-1 gcc) → stage 2 gcc (linked against new glibc, can build full apps)**. Each stage knows where to find the previous; the directories, prefix, and `--with-sysroot` flags must agree. Get any of these wrong and the linker can't find libc, or gcc looks in `/usr/lib` instead of the cross sysroot. crosstool-NG hides this complexity in menuconfig + sequenced builds.
